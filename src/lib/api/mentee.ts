...
export const getYesterdayFeedback = async (
  ...
  const response = await apiClient.get<ApiResponse<YesterdayFeedbackResponse>>(
...
export const completePlanner = async (
  ...
  const response = await apiClient.post<ApiResponse<CompletePlannerResponse>>(
    `/mentee/planner/${date}/complete`,
    { tasks },
  );

  if (!response.data.success || !response.data.data) {
    throw new Error(response.data.message || "플레너 마감에 실패했습니다.");
  }
...
export const createZoomMeeting = async (
  ...
  if (!response.data.success || !response.data.data) {
    throw new Error(response.data.message || "Zoom 미팅 신청에 실패했습니다.");
  }
